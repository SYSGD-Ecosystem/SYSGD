...
export class CryptoPaymentService {
   ...
   */
  async getUserPaymentHistory(userAddress: string, limit: number = 10): Promise<string[]> {
    try {
      return await this.paymentGatewayContract.getUserPayments(userAddress, limit);
    } catch (error) {
...
export function createCryptoPaymentService(network: "sepolia" | "bsc-testnet" | "bsc" | "mainnet"): CryptoPaymentService {
  const configs: Record<string, CryptoConfig> = {
    ...
    "bsc-testnet": {
      ...
      paymentGatewayAddress: process.env.BSC_TESTNET_PAYMENT_GATEWAY_ADDRESS || ""
    },
    bsc: {
      rpcUrl: process.env.BSC_RPC_URL || "https://bsc-dataseed.binance.org",
      chainId: 56,
      testUsdtAddress: process.env.BSC_USDT_ADDRESS || "0x55d398326f99059fF775485246999027B3197955",
      paymentGatewayAddress: process.env.BSC_PAYMENT_GATEWAY_ADDRESS || ""
    },
    mainnet: {
      rpcUrl: process.env.MAINNET_RPC_URL || "https://eth.llamarpc.com",
      chainId: 1,
      testUsdtAddress: process.env.ETH_USDT_ADDRESS || "0xdAC17F958D2ee523a2206206994597C13D831ec7",
