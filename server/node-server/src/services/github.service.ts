...
export class GitHubService {
	...
	async getPullRequests(
		...
		try {
			...
			const pullRequestsBase: GitHubPullRequest[] = data.map((pr) => ({
				...
				user: {
					login: pr.user?.login ?? "unknown", // fallback a "unknown"
					id: pr.user?.id ?? 0, // fallback a 0
					avatar_url: pr.user?.avatar_url ?? "", // fallback a string vacío
					html_url: pr.user?.html_url ?? "", // fallback a string vacío
				},
			...
			const pullRequests = await this.enrichPullRequestsWithStats(
				owner,
				repo,
				pullRequestsBase,
			);
			...
			let totalCount = await this.getTotalPullRequestsCount(
				owner,
				repo,
				filters.state,
			);
	...
	async getRepositoryMetrics(
		...
		try {
			...
			const openPRs = await this.getPullRequests(owner, repo, 1, 100, {
				state: "open",
			});
			const closedPRs = await this.getPullRequests(owner, repo, 1, 100, {
				state: "closed",
			});
			...
			const averagePRSize =
				allPRs.length > 0
					? (totalAdditions + totalDeletions) / allPRs.length
					: 0;
			const mergeRate =
				closedPRs.pullRequests.length > 0
