...
export class GitHubController {
	...
	private static async getUserIdFromRequest(
		req: Request,
	): Promise<string | null> {
	...
	private static async resolveConfig(req: Request) {
		...
		if (body.projectId) {
			const { projectId } = githubConfigLookupSchema.parse({
			...
			if (!saved) {
				const err: any = new Error(
					"GitHub configuration not found for this project",
				);
				err.statusCode = 404;
				throw err;
			}
		...
		throw new Error(
			"Missing GitHub configuration. Provide {owner, repo, token} or {projectId}.",
		);
	}
	...
	static async saveProjectConfig(req: Request, res: Response) {
		try {
			...
			const { projectId, owner, repo } = githubConfigByProjectSchema.parse(
				req.body,
			);
			await GitHubController.assertProjectAccess(projectId, userId);
        ...
        `,
			...
			res.json({ saved: true, configuration: result.rows[0] });
			return;
		} catch (error: any) {
			const status = error?.statusCode || 400;
	...
	static async getProjectConfig(req: Request, res: Response) {
        ...
        `,
			...
			res.json({ configured: true, configuration: result.rows[0] });
			return;
		} catch (error: any) {
			const status = error?.statusCode || 400;
	...
	static async deleteProjectConfig(req: Request, res: Response) {
		try {
			...
			await pool.query(
				"DELETE FROM github_project_config WHERE project_id = $1",
				[projectId],
			);
			res.json({ deleted: true });
			return;
