...
const ExpenseRegister: FC = () => {
		...
		try {
			...
			if (status === 402) {
				toast({
					...
					description:
						"No tienes créditos para generar el PDF desde el servidor",
					variant: "destructive",
				});
				return;
			}
	...
	const annualIngresos = monthTotalsIngresos.reduce(
		(acc, value) => acc + value,
		0,
	);
		...
		>
			<colgroup>
				{monthCodes.flatMap((month) => [
					<col
						key={`${month}-day-col`}
						style={{ width: `${DAY_COLUMN_WIDTH_PX}px` }}
					/>,
					<col
						key={`${month}-month-col`}
						style={{ width: `${MONTH_COLUMN_WIDTH_PX}px` }}
					/>,
				])}
			</colgroup>
			<thead>
				...
				<tr>
					{monthCodes.map((month, idx) => (
						<>
							<th
								key={`${month}-day`}
								className="border py-0.5 px-0 bg-slate-100 dark:bg-slate-800 text-[10px]"
							>
								D
							</th>
							<th
			...
			<tbody>
				...
				<tr>
					...
					{monthCodes.map((month, idx) => (
						<>
							...
							<td
								key={`${month}-total`}
								className="border p-2 text-right font-semibold text-[12px]"
							>
				...
				<tr>
					...
					<td
						colSpan={6}
						className="border p-2 text-right font-bold text-[12px]"
					>
						{annual.toFixed(2)}
					</td>
				</tr>
	...
	const sheetPreview = () => {
		if (activeSheet === "GENERALES") {
			return (
				<table
					id="myTable"
					className="w-full min-w-225 border-collapse text-xs md:text-sm"
				>
					<tbody>
						<tr>
						...
						<tr>
							<td colSpan={2} className="border p-0">
								<Input
									value={generalData.anio}
									onChange={(e) => handleGeneralChange("anio", e.target.value)}
						...
						<tr>
							...
							<td colSpan={2} className="border p-0">
								<Input
									value={generalData.nit}
									onChange={(e) => handleGeneralChange("nit", e.target.value)}
									className="h-8 border-0 rounded-none"
								/>
							</td>
						</tr>
						<tr>
							<td colSpan={8} className="border p-2">
								Domicilio fiscal: (lugar donde desarrolla la actividad): calle,
								No, apto, entre calles:
							</td>
						</tr>
						<tr>
							<td colSpan={8} className="border p-0">
								<Input
									value={generalData.fiscalCalle}
									onChange={(e) =>
										handleGeneralChange("fiscalCalle", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
						...
						<tr>
							...
							<td colSpan={2} className="border p-0">
								<Input
									value={generalData.fiscalMunicipio}
									onChange={(e) =>
										handleGeneralChange("fiscalMunicipio", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
							...
							<td colSpan={2} className="border p-0">
								<Input
									value={generalData.fiscalProvincia}
									onChange={(e) =>
										handleGeneralChange("fiscalProvincia", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
						...
						<tr>
							<td colSpan={8} className="border p-0">
								<Input
									value={generalData.legalCalle}
									onChange={(e) =>
										handleGeneralChange("legalCalle", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
						...
						<tr>
							...
							<td colSpan={2} className="border p-0">
								<Input
									value={generalData.legalMunicipio}
									onChange={(e) =>
										handleGeneralChange("legalMunicipio", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
							...
							<td colSpan={2} className="border p-0">
								<Input
									value={generalData.legalProvincia}
									onChange={(e) =>
										handleGeneralChange("legalProvincia", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
						...
						<tr>
							...
							<td colSpan={5} className="border p-0">
								<Input
									value={generalData.actividad}
									onChange={(e) =>
										handleGeneralChange("actividad", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
							...
							<td className="border p-0">
								<Input
									value={generalData.codigo}
									onChange={(e) =>
										handleGeneralChange("codigo", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
						...
						<tr>
							...
							<td className="border p-0">
								<Input
									value={generalData.firmaDia}
									onChange={(e) =>
										handleGeneralChange("firmaDia", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
							<td className="border p-0">
								<Input
									value={generalData.firmaMes}
									onChange={(e) =>
										handleGeneralChange("firmaMes", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
							<td className="border p-0">
								<Input
									value={generalData.firmaAnio}
									onChange={(e) =>
										handleGeneralChange("firmaAnio", e.target.value)
									}
									className="h-8 border-0 rounded-none"
								/>
							</td>
		...
		return (
			<table
				id="myTable"
				className="w-full min-w-325 border-collapse text-xs"
			>
				<thead>
					<tr>
						<th
							colSpan={16}
							className="border p-2 bg-slate-200 dark:bg-slate-700"
						>
					...
					<tr>
						...
						<th colSpan={9} className="border p-2">
				...
				<tbody>
							...
							{(
								...
								<td key={`${row.mes}-${field}`} className="border p-0">
									<Input
										value={row[field]}
										onChange={(e) =>
											handleTributoChange(idx, field, e.target.value)
										}
										className="h-8 border-0 rounded-none text-right"
									/>
								</td>
							))}
							<td className="border p-2 text-right font-semibold text-[12px]">
								{row.g.toFixed(2)}
							</td>
							<td className="border p-2 text-right font-semibold text-[12px]">
								{row.k.toFixed(2)}
					...
					<tr className="font-bold bg-slate-100 dark:bg-slate-800">
						...
						<td className="border p-2 text-right">
							{tributosTotals.b.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.c.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.d.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.e.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.f.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.h.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.i.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.j.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.l.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.m.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.n.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.o.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.p.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.g.toFixed(2)}
						</td>
						<td className="border p-2 text-right">
							{tributosTotals.k.toFixed(2)}
						</td>
					</tr>
					{instructions.map((text, idx) => (
						<tr key={`instruction-${idx + 1}`}>
							<td
								colSpan={16}
								className="border p-2 text-left whitespace-pre-wrap"
							>
								{text}
							</td>
						</tr>
				...
				<div className="mb-4 flex flex-wrap items-center justify-between gap-3">
					<div className="flex items-center gap-2">
						<h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">
							Registro de Ingresos y Gastos TCP
						</h1>
					</div>
					<div className="flex items-center gap-2">
						<Select
							value={pageSize}
							onValueChange={(value) => setPageSize(value as "A4" | "Carta")}
						>
				...
				<div className="grid grid-cols-1 gap-4">
					<Card className="">
						<CardHeader className="pb-2">
							<CardTitle className="text-sm">Hojas</CardTitle>
						</CardHeader>
						<CardContent className="flex gap-2">
							{(
								["GENERALES", "INGRESOS", "GASTOS", "TRIBUTOS"] as SheetTab[]
							).map((sheet) => (
								<Button
									key={sheet}
						...
						{(activeSheet === "INGRESOS" || activeSheet === "GASTOS") && (
							<Card>
								...
								<CardContent className="grid grid-cols-1 md:grid-cols-5 gap-3">
									...
									<div>
										<Label>
											{activeSheet === "INGRESOS" ? "Ingreso" : "Gasto"}
										</Label>
										<Input
						...
						<Card id="print-area">
							<CardHeader className="pb-2">
								<CardTitle className="text-sm">
									Vista previa imprimible ({pageSize})
								</CardTitle>
							</CardHeader>
							<CardContent>
								<div className="overflow-x-auto">
									<div className="bg-white dark:bg-slate-900 border dark:border-slate-700 rounded shadow p-2 min-w-276">
										{sheetPreview()}
									</div>
								</div>
							</CardContent>
