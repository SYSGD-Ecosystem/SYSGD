...
const MonthlyChart: FC<{ analysis: WorkspaceAnalysis }> = ({ analysis }) => {
	const maxValue = Math.max(1, ...analysis.monthly.flatMap((month) => [month.ingresos, month.gastos, Math.abs(month.neto)]));
						...
						<div key={month.month} className="grid grid-cols-[3rem_1fr_5.5rem] items-center gap-3 text-sm">
							...
							<div className="space-y-1">
								<div className="h-2 rounded-full bg-slate-100 dark:bg-slate-800">
									<div className="h-2 rounded-full bg-emerald-500" style={{ width: `${Math.max(2, (month.ingresos / maxValue) * 100)}%` }} />
								</div>
								<div className="h-2 rounded-full bg-slate-100 dark:bg-slate-800">
									<div className="h-2 rounded-full bg-rose-500" style={{ width: `${Math.max(2, (month.gastos / maxValue) * 100)}%` }} />
								</div>
							</div>
...
export const GeneralView: FC<{ workspace: CloudWorkspaceEntry; analysis: WorkspaceAnalysis }> = ({ workspace, analysis }) => {
		...
		<div className="grid gap-4 xl:grid-cols-[1fr_22rem]">
			...
			<Card className="rounded-lg shadow-sm">
				...
				<CardContent className="space-y-3 p-4 pt-0">
					<MetricCard title="Completitud fiscal" value={`${analysis.completenessScore}%`} detail="Campos base completados" icon={<WalletCards />} accent="bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300" />
...
export const EntriesView: FC<{ workspace: CloudWorkspaceEntry; type: "ingresos" | "gastos" }> = ({ workspace, type }) => {
...
export const TributosView: FC<{ workspace: CloudWorkspaceEntry; analysis: WorkspaceAnalysis }> = ({ workspace, analysis }) => (
	<div className="space-y-4">
		<div className="grid gap-4 md:grid-cols-3">
			<MetricCard title="Tributos deducibles" value={formatMoney(analysis.totalTributos)} detail="Ventas, fuerza, sellos, CSS y cuota" icon={<Landmark />} accent="bg-sky-100 text-sky-700 dark:bg-sky-500/15 dark:text-sky-300" />
			<MetricCard title="Otros deducibles" value={formatMoney(analysis.totalOtrosDeducibles)} detail="Restauracion, arrendamiento y otros" icon={<WalletCards />} accent="bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300" />
			<MetricCard title="Meses declarados" value={String(analysis.tributoRows)} detail="Filas con importes registrados" icon={<CalendarDays />} accent="bg-violet-100 text-violet-700 dark:bg-violet-500/15 dark:text-violet-300" />
		</div>
		<Card className="rounded-lg shadow-sm">
			<CardHeader className="p-4">
					...
					<table className="w-full min-w-[820px] text-sm">
						...
						<tbody>
							{MONTH_CODES.map((month, index) => {
								const total = tributoTotal(getTributoRow(workspace.registro, index));
								return (
									<tr key={month} className="border-b border-slate-100 dark:border-slate-800/80">
										<td className="py-3 pr-4 font-medium text-slate-950 dark:text-slate-50">{month}</td>
										<td className="py-3 pr-4 text-right font-semibold">{formatMoney(total)}</td>
									</tr>
								);
							})}
						</tbody>
...
export const ResumenView: FC<{ analysis: WorkspaceAnalysis }> = ({ analysis }) => (
	<div className="grid gap-4 xl:grid-cols-[1fr_24rem]">
		<MonthlyChart analysis={analysis} />
		<div className="space-y-4">
			<MetricCard title="Base imponible" value={formatMoney(analysis.baseImponible)} detail="Ingresos menos gastos y deducciones" icon={<LayoutDashboard />} accent="bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300" />
			<MetricCard title="Impuesto estimado" value={formatMoney(analysis.impuestoEstimado)} detail="Estimacion referencial al 15%" icon={<Landmark />} accent="bg-sky-100 text-sky-700 dark:bg-sky-500/15 dark:text-sky-300" />
			<MetricCard title="Operaciones registradas" value={String(analysis.incomeRows + analysis.expenseRows)} detail={`${analysis.incomeRows} ingresos / ${analysis.expenseRows} gastos`} icon={<Archive />} accent="bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300" />
		</div>
	</div>
);

export const InventoryView: FC<{ workspace: CloudWorkspaceEntry }> = ({ workspace }) => {
					...
					<div className="space-y-2">
						{operations.map((operation) => (
							<div key={operation.id} className="rounded-md border border-slate-200 p-3 dark:border-slate-800">
								...
								<p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{operation.fecha} / {operation.cantidad} {operation.unidad} / {formatMoney(operation.total)}</p>
							</div>
						))}
...
export const TercerosView: FC<{ workspace: CloudWorkspaceEntry }> = ({ workspace }) => {
...
export const BackupView: FC<{ onReload: () => void; onDownloadBackup: () => void; canDownload: boolean }> = ({
	onReload,
	onDownloadBackup,
	canDownload,
}) => (
	<div className="space-y-4">
		<Card className="rounded-lg shadow-sm">
			<CardHeader className="flex-row items-center justify-between gap-3 p-4">
				...
				<Button variant="outline" size="sm" onClick={onDownloadBackup} disabled={!canDownload}>
					<FileDown className="size-4" />
					Descargar respaldo
				</Button>
			</CardHeader>
			<CardContent className="p-4 pt-0 text-sm text-slate-600 dark:text-slate-300">
				Descarga una copia local del ledger actual o restaura un respaldo exportado por la app.
			</CardContent>
		</Card>
...
export const SupportView: FC<{ view: GcTcpView }> = ({ view }) => {
	const content: Record<"documentos" | "seguridad" | "licencias" | "acerca" | "ayuda" | "recursos" | "nomencladores", { title: string; description: string; icon: JSX.Element }> = {
		documentos: {
			title: "Documentos",
