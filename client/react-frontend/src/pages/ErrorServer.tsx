...
import { AlertTriangle, RefreshCw, Home, ArrowLeft, Wifi, Server, Clock, HelpCircle } from "lucide-react"
...
const ErrorServer: React.FC<ErrorServerProps> = ({
  ...
  const [connectionStatus, setConnectionStatus] = useState<"checking" | "offline" | "online">("checking")
      ...
      <div className="relative z-10 w-full max-w-2xl">
        ...
        <Card className="bg-slate-800/90 backdrop-blur-sm border-slate-700 shadow-2xl">
          <CardHeader className="text-center pb-4">
            ...
            <CardTitle className="text-3xl font-bold text-white mb-2">¡Oops! Algo salió mal</CardTitle>

            <p className="text-xl text-red-400 mb-4">{errorMessage}</p>

            <p className="text-slate-400 max-w-md mx-auto">
              No pudimos conectar con nuestros servidores. Esto puede ser temporal, por favor intenta nuevamente en unos
              momentos.
            </p>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
            ...
            {retryCount > 0 && (
              <Alert className="bg-yellow-500/10 border-yellow-500/20">
                ...
                <AlertDescription className="text-yellow-200">Intentos de reconexión: {retryCount}</AlertDescription>
              </Alert>
            )}
            ...
            <div className="text-center space-y-3">
              ...
              <div className="flex flex-wrap justify-center gap-4 text-sm">
                <button className="text-blue-400 hover:text-blue-300 underline">Reportar problema</button>
                <button className="text-blue-400 hover:text-blue-300 underline">Estado del servicio</button>
                <button className="text-blue-400 hover:text-blue-300 underline">Contactar soporte</button>
              </div>
            </div>

            {/* Technical Details (Collapsible) */}
            <details className="bg-slate-700/30 rounded-lg p-4">
              <summary className="text-slate-300 cursor-pointer hover:text-white">Detalles técnicos</summary>
