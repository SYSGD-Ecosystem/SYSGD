...
export function IdeasBank() {
	...
	const filteredIdeas = ideas.filter(
		(idea) => filterStatus === "Todas" || idea.estado === filterStatus,
	);
	...
	const handleVote = (ideaId: number, increment: boolean) => {
		setIdeas(
			ideas.map((idea) =>
				idea.id === ideaId
					? { ...idea, votos: idea.votos + (increment ? 1 : -1) }
					: idea,
			),
		);
	};
									...
									<div className="flex items-start gap-3">
										...
										<div>
											...
											<div className="flex gap-2 mt-2">
												<Badge className={getStatusColor(idea.estado)}>
													{idea.estado}
												</Badge>
												<Badge variant={getPriorityColor(idea.prioridad)}>
													{idea.prioridad}
												</Badge>
											</div>
										</div>
						...
						<div className="space-y-4">
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setEditingIdea({ ...editingIdea, titulo: e.target.value })
									}
								...
								<div>
									...
									<select
										...
										onChange={(e) =>
											setEditingIdea({
												...editingIdea,
												implementabilidad: e.target.value,
											})
										}
										className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
									>
