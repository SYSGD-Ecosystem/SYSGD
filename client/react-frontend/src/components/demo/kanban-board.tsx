...
export function KanbanBoard() {
  ...
  const handleDrop = (e: React.DragEvent, newStatus: string) => {
    ...
    if (draggedTask && draggedTask.estado !== newStatus) {
      setTasks(tasks.map((task) => (task.id === draggedTask.id ? { ...task, estado: newStatus } : task)))
    }
    setDraggedTask(null)
  }
    ...
    <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm">
      <div className="p-4 md:p-6 border-b border-gray-200 dark:border-gray-700">
        <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4 gap-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white mb-2">TABLERO KANBAN</h1>
            <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-4">
              VISUALIZACIÓN DE TAREAS POR ESTADO
            </h2>
          </div>
          <div className="text-right">
            <div className="text-sm font-medium text-gray-600 dark:text-gray-400">KB1</div>
          </div>
        </div>
      </div>
                  ...
                  >
                    <CardHeader className="pb-2">
                      <div className="flex items-start justify-between">
                        ...
                        <Badge variant={getPriorityColor(task.prioridad)} className="text-xs ml-2 flex-shrink-0">
                          {task.prioridad}
                        </Badge>
                      </div>
                    </CardHeader>
                    <CardContent className="pt-0">
                      <p className="text-xs text-gray-600 dark:text-gray-400 mb-3 line-clamp-2">{task.descripcion}</p>
