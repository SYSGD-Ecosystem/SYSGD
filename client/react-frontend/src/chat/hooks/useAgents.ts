...
import { Agent, CreateAgentRequest, UpdateAgentRequest, AgentMessageRequest } from '../../types/Agent';
...
export const useAgents = () => {
  ...
  const createAgent = async (agentData: CreateAgentRequest): Promise<Agent | null> => {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(API_BASE_URL, {
  ...
  const updateAgent = async (id: string, agentData: UpdateAgentRequest): Promise<Agent | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const sendMessageToAgent = async (messageData: AgentMessageRequest) => {
    ...
    try {
      ...
      const agentRequest = {
        ...
        ...(messageData.attachment_type === 'image' && messageData.attachment_url && { 
          image: messageData.attachment_url 
        }),
        ...(messageData.attachment_type === 'audio' && messageData.attachment_url && { 
          audio: messageData.attachment_url 
        }),
        ...(messageData.attachment_type === 'video' && messageData.attachment_url && { 
          video: messageData.attachment_url 
        }),
        ...(messageData.attachment_type === 'file' && messageData.attachment_url && { 
      ...
      if (!serverResponse.ok) {
        ...
        throw new Error(errorData.error || 'Error al guardar mensajes en el servidor');
      }
