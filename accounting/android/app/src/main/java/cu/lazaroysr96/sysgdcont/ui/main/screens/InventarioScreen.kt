...
@Composable
private fun HistorialContent(viewModel: InventarioViewModel, facturaViewModel: FacturaViewModel, padding: PaddingValues, canUseProFeatures: Boolean) {
    val uiState by viewModel.uiState.collectAsStateWithLifecycle()
        ...
        Card(modifier = Modifier.fillMaxWidth().padding(16.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.primaryContainer)) {
            Column(modifier = Modifier.fillMaxWidth().padding(16.dp)) {
                ...
                Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    if (!canUseProFeatures) Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) { Text("La generación de facturas requiere plan Pro o VIP.", modifier = Modifier.padding(12.dp), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
        ...
        if (uiState.isLoading) { Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) { CircularProgressIndicator() } }
            ...
            LazyColumn(modifier = Modifier.fillMaxSize(), contentPadding = PaddingValues(horizontal = 16.dp, vertical = 8.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                fechas.forEach { fecha ->
                    ...
                    if (ventas.isNotEmpty()) item(key = "ventas-$fecha") { DiaVentasCard(fecha, ventas, viewModel::getDetalleOperacion, { viewModel.anularVenta(it) }, { venta, lineas -> facturaViewModel.showFacturaDialog(venta, lineas) }, canUseProFeatures) }
