...
constructor(
        ...
        companion object {
                ...
                private val LAST_DOWNLOADED_VERSION_KEY =
                        stringPreferencesKey("last_downloaded_version")
        ...
        val lastSync: Flow<String?> =
                context.ledgerDataStore.data.map { prefs -> prefs[LAST_SYNC_KEY] }
        ...
        val cuentasIngreso: Flow<List<CuentaContable>> =
                cuentaContableDao.observeByTipoCuenta(TipoCuenta.INGRESO)

        //     val cuentasGasto: Flow<List<CuentaContable>> =
        //             cuentaContableDao.observeByTipoNaturaleza(TipoCuenta.GASTO,
        // NaturalezaCuenta.DEUDORA)

        val cuentasGasto: Flow<List<CuentaContable>> =
                cuentaContableDao.observeByTipoCuenta(TipoCuenta.GASTO)
        ...
        private fun parseWorkspaceProfiles(raw: String?): List<WorkspaceProfile> {
                return if (raw.isNullOrBlank()) {
                        ...
                        runCatching {
                                        gson.fromJson(raw, Array<WorkspaceProfile>::class.java)
                                                ?.toList()
                                                .orEmpty()
                                }
        ...
        private suspend fun buildActiveWorkspaceSnapshot(): WorkspaceSnapshot {
                ...
                val baselineRegistro =
                        prefs[BASELINE_REGISTRO_KEY]?.let { raw ->
                                runCatching { gson.fromJson(raw, RegistroTCP::class.java) }
                                        .getOrNull()
                        }
                val baselineInventario =
                        prefs[BASELINE_INVENTARIO_KEY]?.let { raw ->
                                runCatching { gson.fromJson(raw, InventarioRegistro::class.java) }
                                        .getOrNull()
                        }
                return WorkspaceSnapshot(
        ...
        private fun parseCloudLedgerContainer(json: String): CloudLedgerContainer? {
                val rawContainer =
                        runCatching { gson.fromJson(json, RawCloudLedgerContainer::class.java) }
                                .getOrNull()
                                ?: return null
        ...
        private suspend fun applyCloudLedgerContainer(
                ...
                return try {
                        ...
                        val activeId =
                                container.activeWorkspaceId.takeIf { id ->
                        ...
                        context.ledgerDataStore.edit { prefs ->
                                ...
                                snapshots.forEach { (workspaceId, snapshot) ->
                                        prefs[workspaceSnapshotKey(workspaceId)] =
                        ...
                        val activeSnapshot =
                                snapshots[activeId]
                                        ?: cloudEntryToSnapshot(
                                                normalizedEntries.first(),
                                                serverVersion
                                        )
        ...
        private suspend fun applyWorkspaceSnapshot(snapshot: WorkspaceSnapshot) {
                ...
                context.ledgerDataStore.edit { prefs ->
                        ...
                        if (snapshot.baselineRegistro != null) {
                                prefs[BASELINE_REGISTRO_KEY] =
                                        gson.toJson(snapshot.baselineRegistro)
                        } else {
                                prefs.remove(BASELINE_REGISTRO_KEY)
                        }
                        if (snapshot.baselineInventario != null) {
                                prefs[BASELINE_INVENTARIO_KEY] =
                                        gson.toJson(snapshot.baselineInventario)
                        } else {
        ...
        suspend fun ensureDefaultAccounts() {
                ...
                val faltantes =
                        CuentasContablesPorDefecto.todas().filter { existentes[it.codigo] == null }
        ...
        suspend fun switchWorkspace(workspaceId: String) {
                ...
                require(profiles.any { it.id == workspaceId }) {
        ...
        suspend fun updateTributoConfig(
                ...
                val actual =
                        tributoConfigDao.getAll().firstOrNull { it.key == key }
                                ?: TributoConfigsPorDefecto.entidades().firstOrNull {
                                        it.key == key
                                }
                                        ?: return
                ...
                val cuentas =
                        cuentaIds.map { it.trim() }.filter { it.isNotBlank() }.distinct().map {
                                cuentaId ->
        ...
        private fun buildRemoteRegistro(response: ContLedgerResponse): RegistroTCP? {
                ...
                val registro =
                        runCatching { gson.fromJson(registroElement, RegistroTCP::class.java) }
                                .getOrNull()
                                ?: return null
                ...
                val inventario =
                        response.inventarioRegistro?.let { inventarioElement ->
                                runCatching {
                                                gson.fromJson(
                                                        inventarioElement,
                                                        InventarioRegistro::class.java
                                                )
                                        }
                                        .getOrNull()
                        }
                val combinado =
                        if (inventario != null) registro.copy(inventario = inventario) else registro
                ...
                withContext(Dispatchers.IO) {
                        try {
                                ...
                                val importedContainer: CloudLedgerContainer? =
                                        try {
                                                ...
                                                if (rootObject.has("container")) {
                                                        ...
                                                        val registroElement =
                                                                rootObject.get("registro")
                                                        val legacyRegistro =
                                                                tryParseLegacyRegistro(
                                                        ...
                                                        val legacyRegistro =
                                                                tryParseLegacyRegistro(cleanedJson)
                                                        if (legacyRegistro != null) {
                                                                CloudLedgerContainer(
                                                                        activeWorkspaceId =
                                                                                DEFAULT_WORKSPACE_ID,
                                ...
                                val container =
                                        importedContainer
                                                ?: throw Exception(
                                                        "No se pudo interpretar el backup"
                                                )
                                applyCloudLedgerContainer(container, serverVersion = "")
                                        .getOrThrow()
                                                ...
                                                OperacionInventario(
                                                        id = op.id?.sanitizeId()
                                                                        ?: java.util
                                                                                ...
                                                                                .randomUUID()
                                                                                .toString(),
                                                        tipo = op.tipo?.lowercase()
                                                                        ?: return@mapNotNull null,
                                                        fecha = op.fecha?.sanitizeText()
                                                                        ?: return@mapNotNull null,
        ...
        suspend fun uploadLocalToRemote(): Result<SyncResult> {
                return try {
                        ...
                        if (!updateResponse.isSuccessful) {
                                ...
                                return Result.failure(
                                        Exception("Error al subir datos: ${updateResponse.code()}")
                                )
                        }
        ...
        suspend fun uploadMergedToRemote(mergedRegistro: RegistroTCP): Result<SyncResult> {
                return try {
                        ...
                        if (!updateResponse.isSuccessful) {
                                ...
                                return Result.failure(
                                        Exception("Error al subir merge: ${updateResponse.code()}")
                                )
                        }
        ...
        suspend fun registrarOperacionRapida(
                ...
                saveRegistroAplicandoTributos(
                        current.copy(
                                ingresos =
                                        ingresos.mapValues { (_, entries) ->
                                                entries.sortedBy { row ->
                                                        row.dia.toIntOrNull() ?: 0
                                                }
                                        },
                                gastos =
                                        gastos.mapValues { (_, entries) ->
                                                entries.sortedBy { row ->
                                                        row.dia.toIntOrNull() ?: 0
                                                }
                                        }
        ...
        private suspend fun updateEntry(
                ...
                if (newDia in 1..31 && importe > 0) {
                        val entryId =
                                previousEntry?.id?.takeIf { it.isNotBlank() }
                                        ?: UUID.randomUUID().toString()
                        monthEntries.add(
                                DayAmountRow(
                                        entryId,
        ...
        private suspend fun addEntry(
                ...
                val entryId = UUID.randomUUID().toString()
                monthEntries.add(
                        DayAmountRow(entryId, dia.toString(), String.format("%.2f", importe))
                )
                saveEntryMetadata(entryId, month, type, cuenta, nota)
        ...
        private suspend fun updateEntryById(
                ...
                Log.e(
                        "SYSGD_EDIT",
                        "updateEntryById llamado — type=$type, entryId='$entryId', month=$month"
                )
                ...
                Log.e(
                        "SYSGD_EDIT",
                        "IDs disponibles en $type: ${sourceEntries.values.flatten().map { it.id }}"
                )
                ...
                val previousEntry =
                        monthEntriesOrigen.firstOrNull { it.id == entryId } /*?:  return*/
                ...
                if (previousEntry == null) {
                        Log.e(
                                "SYSGD_EDIT",
                                "ERROR — entryId '$entryId' no encontrado en ningún mes"
                        )
                        return
                }
                ...
                if (newDia !in 1..31 || importe <= 0.0) {
                        ...
                        val monthEntriesDestino =
                                sourceEntries[month]?.toMutableList() ?: mutableListOf()
                        ...
                        sourceEntries[month] =
                                monthEntriesDestino.sortedBy { it.dia.toIntOrNull() ?: 0 }
                        saveEntryMetadata(entryId, month, type, cuenta, nota)
                }
                ...
                return try {
                        ...
                        if (!shouldSync) {
                                return Result.success(
                                        SyncResult(
                                                true,
                                                "Sincronización automática omitida",
                        ...
                        val result =
                                when {
                                        remoteContainer != null -> {
                                                applyCloudLedgerContainer(
                                                        remoteContainer,
                                                        remote.updatedAt.orEmpty()
                                                )
                                        }
                                        localContainer.workspaces.isNotEmpty() ||
                                                isLocalModified() -> {
                                                uploadLocalToRemote()
                                        }
                                        else -> {
        ...
        suspend fun sync(): Result<SyncResult> {
                return try {
                        ...
                        when {
                                ...
                                !localModified &&
                                        (!hasBaseline || !hasLocalData || serverChanged) -> {
                                        Result.success(
                                                SyncResult(
                                                        success = true,
                                                        message =
                                ...
                                localModified && serverChanged -> {
                                        val identicalContainers =
                                                remoteContainer?.let {
                                                        cloudContainersEqual(localContainer, it)
                                                } == true
        ...
        private fun checkForConflicts(
                ...
                if (remote == null) {
                        return ConflictInfo(
                                ...
                                mergePossible = true,
                                conflictMessage =
                                        "No hay datos remotos, se puede subir versión local."
                        )
                }
                ...
                return if (conflicts.isNotEmpty()) {
                        ...
                        ConflictInfo(
                                ...
                                mergePossible = true,
                                conflictMessage =
                                        "No hay conflictos, se puede hacer merge automático"
                        )
                }
        }
        ...
        suspend fun mergeVersions(
                ...
                val mergedTributos =
                        if (remote.tributos.isNotEmpty()) remote.tributos else local.tributos

                val mergedGenerales =
                        if (remote.generales.nombre.isNotEmpty()) remote.generales
                        else local.generales

                val remoteInventarioChanged =
                        !inventoriesEqual(remote.inventario, baselineInventario)
                val localInventarioChanged = !inventoriesEqual(local.inventario, baselineInventario)
                val mergedInventario =
                        when {
                                remoteInventarioChanged && !localInventarioChanged ->
                                        remote.inventario
                                !remoteInventarioChanged && localInventarioChanged ->
                                        local.inventario
                                remoteInventarioChanged && localInventarioChanged ->
        ...
        private suspend fun applyAutoCalculatedTributos(registro: RegistroTCP): RegistroTCP {
                val configs =
                        tributoConfigDao.getAll().ifEmpty { TributoConfigsPorDefecto.entidades() }
                val relaciones = tributoCuentaBaseDao.getAll()
                val relacionesPorTributo = relaciones.groupBy { it.tributoKey }
                val cuentaPorAsientoId =
                        ingresoGastoCuentaDao.getAll().associate {
                                it.ingresoGastoId to it.cuentaId
                        }
                        ...
                        LedgerConstants.MONTHS.mapIndexed { index, month ->
                                val actual =
                                        registro.tributos.getOrNull(index)
                                                ?: TributoRow(
                                                        mes = LedgerConstants.monthLabels[month]
                                                                        ?: month
                                                )
                                val base =
                                ...
                                configs.fold(base) { row, config ->
                                        ...
                                        when {
                                                !config.incluido ->
                                                        updateTributoValue(row, config.key, "")
                                                config.autocalcular -> {
                                                        val baseImponible =
                                                                calculateBaseImponible(
                                                                        registro,
        ...
        private fun calculateBaseImponible(
                ...
                val total =
                        ingresos.sumOf { row ->
                                ...
                                gastos.sumOf { row ->
                                        val cuentaId = cuentaPorAsientoId[row.id]
                                        if (cuentaId in cuentaIds) parseCurrency(row.importe)
                                        else 0.0
                                }
                return round2(total)
        }
        ...
        private fun computeAccountBalances(
                ...
                fun acumular(rows: Collection<List<DayAmountRow>>, tipoMovimiento: String) {
                        rows.flatten().forEach { row ->
                                ...
                                acumulado[cuenta.id] =
                                        round2((acumulado[cuenta.id] ?: 0.0) + importe * signo)
                        }
                }
                ...
                val hijosPorPadre =
                        cuentas.filter { !it.padreId.isNullOrBlank() }.groupBy { it.padreId!! }
        ...
        fun calculateAnnualReport(registro: RegistroTCP): AnnualReport {
                val monthly =
                        LedgerConstants.MONTHS.map { month ->
                                ...
                                val otros =
                                        if (tribIndex < registro.tributos.size) {
                                                otrosDeduciblesSubtotal(
                                                        registro.tributos[tribIndex]
                                                )
                                        } else 0.0
        ...
        private fun tributosSubtotal(item: TributoRow): Double {
                val keys =
                        listOf("ventas", "fuerza", "sellos", "anuncios", "css20", "css14", "otros")
                return round2(
                        keys.sumOf { key ->
                                when (key) {
        ...
        private fun normalizeImportedRegistro(source: RegistroTCP): RegistroTCP {
                ...
                fun safeText(block: () -> Any?, default: String = ""): String =
                        runCatching { block()?.toString() }.getOrNull()?.trim()?.takeIf {
                                it.isNotBlank()
                        }
                                ?: default
                ...
                val generales =
                        source.generales.copy(
                                ...
                                nombre =
                                        safeText(
                                                { source.generales.nombre },
                                                base.generales.nombre
                                        ),
                                nit = safeText({ source.generales.nit }, base.generales.nit),
                                actividad =
                                        safeText(
                                                { source.generales.actividad },
                                                base.generales.actividad
                                        ),
                                codigo =
                                        safeText(
                                                { source.generales.codigo },
                                                base.generales.codigo
                                        ),
                                fiscalCalle =
                                        safeText(
                        ...
                        productos =
                                source.productos.map { producto ->
                                        ProductoInventario(
                                                ...
                                                nombre =
                                                        safeString({ producto.nombre }, "Producto"),
                        ...
                        catalogoVentas =
                                source.catalogoVentas.map { catalogo ->
                                        CatalogoVentaRegistro(
                                                ...
                                                productoId =
                                                        fallbackId(
                                                                safeString({ catalogo.productoId })
                                                        ),
                                                precioReferencia =
                                                        safeDouble({ catalogo.precioReferencia }),
                                                almacenId =
                                                        safeString(
                                                                { catalogo.almacenId },
                                                                Almacen.DEFAULT_ID
                        ...
                        historialPrecios =
                                source.historialPrecios.map { precio ->
                                        PrecioProductoRegistro(
                                                ...
                                                fechaHasta =
                                                        runCatching { precio.fechaHasta?.trim() }
                                                                .getOrNull()
                        ...
                        stock =
                                source.stock.map { stock ->
                                        StockRegistro(
                                                ...
                                                stockDisponible =
                                                        safeDouble({ stock.stockDisponible }),
                                                modoStock =
                                                        safeString(
                                                                { stock.modoStock },
                                                ...
                                                productosVinculadosIds =
                                                        safeString(
                                                                { stock.productosVinculadosIds },
                                                                "[]"
                                                        ),
                                                ratiosConversion =
                                                        safeString(
                                                                { stock.ratiosConversion },
                                                                "[]"
                                                        ),
                                                ultimaActualizacion =
                                                        safeString({ stock.ultimaActualizacion }),
                                                visibleEnVentas =
                                                        safeBoolean({ stock.visibleEnVentas })
                                        )
                                },
                        movimientos =
                                source.movimientos.map { movimiento ->
                                        MovimientoInventarioRegistro(
                                                ...
                                                tipoMovimiento =
                                                        safeString({ movimiento.tipoMovimiento }),
                                                ...
                                                almacenOrigenId =
                                                        runCatching {
                                                                movimiento.almacenOrigenId?.trim()
                                                        }
                                                                .getOrNull()
                                                                ?.takeIf { it.isNotBlank() },
                                                almacenDestinoId =
                                                        runCatching {
                                                                movimiento.almacenDestinoId?.trim()
                                                        }
                                                                .getOrNull()
                                                                ?.takeIf { it.isNotBlank() },
                                                stockOrigenAntes =
                                                        safeNullableDouble {
                                                                movimiento.stockOrigenAntes
                                                        },
                                                stockOrigenDespues =
                                                        safeNullableDouble {
                                                                movimiento.stockOrigenDespues
                                                        },
                                                stockDestinoAntes =
                                                        safeNullableDouble {
                                                                movimiento.stockDestinoAntes
                                                        },
                                                stockDestinoDespues =
                                                        safeNullableDouble {
                                                                movimiento.stockDestinoDespues
                                                        },
                                                referenciaId =
                                                        safeString({ movimiento.referenciaId }),
                        ...
                        operaciones =
                                source.operaciones.map { operacion ->
                                        OperacionInventario(
                                                ...
                                                productoId =
                                                        fallbackId(
                                                                safeString({ operacion.productoId })
                                                        ),
                                                nombreProducto =
                                                        safeString(
                                                                { operacion.nombreProducto },
                                                                "Producto"
                                                        ),
                                                unidad = safeString({ operacion.unidad }),
                                                cantidad = safeDouble({ operacion.cantidad }),
                                                precioUnitario =
                                                        safeDouble({ operacion.precioUnitario }),
        ...
        private fun normalizeMonthRows(rows: List<DayAmountRow>?): List<DayAmountRow> {
                ...
                return rows
                        .mapNotNull { row ->
                                ...
                                // DayAmountRow(dia = dia.toString(), importe =
        ...
        private fun emptyRegistro(
                ...
                val emptyMonthEntries =
                        LedgerConstants.MONTHS.associateWith { emptyList<DayAmountRow>() }
                val emptyTributos =
                        LedgerConstants.MONTHS.map { month ->
                                TributoRow(mes = LedgerConstants.monthLabels[month] ?: month)
                        }
                ...
                return withContext(Dispatchers.IO) {
                        try {
                                ...
                                if (response.code() == 502) {
                                        onRetryMessage(
                                                "Servidor dormido. Reintentando en 15 segundos..."
                                        )
                                        delay(15000)
                                        val retryResponse =
                                                apiService.downloadPdf("Bearer $token", pdfPayload)
                                        if (retryResponse.code() == 401 ||
                                                        retryResponse.code() == 403
                                        ) {
                                                authRepository.logout()
                                                return@withContext Result.failure(
                                                        Exception(
        ...
        /**
         * Genera un informe mensual de ingresos y gastos (Estado de Resultados) Estructura del
         * informe:
                ...
                return withContext(Dispatchers.IO) {
                        ...
                        val file =
                                body.byteStream().use { input ->
                                        documentStorageRepository.saveStream(
                                                DocumentCategory.DJ,
                                                fileName,
                                                input
                                        )
        ...
        private fun buildPdfIntent(file: File): Result<Intent> {
                return Result.success(
                        documentStorageRepository.buildViewIntent(file, "application/pdf")
                )
        }
        ...
        private fun generateOfflineTcpPdf(payload: TcpPdfPayload): File {
                ...
                val file =
                        documentStorageRepository.createDocumentFile(DocumentCategory.DJ, fileName)
        ...
        private fun buildHeaderTable(
                ...
                table.addCell(
                        textCell(generalData.anio, boldFont, 11f, TextAlignment.CENTER, colspan = 2)
                )
                table.addCell(
                        textCell(
                ...
                table.addCell(
                        textCell(generalData.nit, regularFont, 10f, TextAlignment.LEFT, colspan = 2)
                )
                table.addCell(
                        textCell(
                ...
                table.addCell(
                        textCell("Municipio:", regularFont, 10f, TextAlignment.LEFT, colspan = 2)
                )
                table.addCell(
                        textCell(
                ...
                table.addCell(
                        textCell("Provincia:", regularFont, 10f, TextAlignment.LEFT, colspan = 2)
                )
                table.addCell(
                        textCell(
        ...
        private fun addMonthSection(
                ...
                val widths =
                        LedgerConstants.MONTHS
                                .flatMap {
                                        listOf(
                                                TCP_MONTH_DAY_COLUMN_WIDTH,
                                                TCP_MONTH_VALUE_COLUMN_WIDTH
                                        )
                                }
                                .toFloatArray()
                val table = Table(widths)
                ...
                LedgerConstants.MONTHS.forEach { month ->
                        ...
                        table.addCell(
                                monthGridCell(
                                        sanitizeMonthAmountCell(
                                                monthTotalText(entries[month].orEmpty())
                                        ),
                                        boldFont,
                                        7f,
                                        TextAlignment.RIGHT,
        ...
        private fun buildTributosRows(tributos: List<TributoPdfRow>): List<OfflineTributoRow> {
                ...
                return LedgerConstants.MONTHS.mapIndexed { index, monthCode ->
                        ...
                        val source =
                                tributosByMonth[normalizeMonth(monthLabel)]
                                        ?: tributos.getOrNull(index)
